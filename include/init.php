...
// Fix for nginx setups (e.g. Mail-in-a-Box) where fastcgi_split_path_info
// strips the path prefix from SCRIPT_NAME, causing form actions to point to
// the wrong URL and POST data to be lost on the resulting 302 redirect.
if (!empty($_SERVER['REQUEST_URI']) && !empty($_SERVER['SCRIPT_NAME'])) {
    $uri_no_qs = strtok($_SERVER['REQUEST_URI'], '?');
    $script_pos = strpos($uri_no_qs, $_SERVER['SCRIPT_NAME']);
    if ($script_pos !== false && $script_pos > 0) {
        $prefix = substr($uri_no_qs, 0, $script_pos);
        $base_uri = $prefix . $base_uri;
