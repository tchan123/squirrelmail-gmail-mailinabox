...
if ($sError) {
   ...
   // PRG: save updated mailbox cache and redirect to GET so the refreshed
   // message list reflects the delete/move without requiring a manual reload.
   $mailbox_cache[$account.'_'.$aMailbox['NAME']] = $aMailbox;
   sqsession_register($mailbox_cache,'mailbox_cache');
   sqimap_logout($imapConnection);
   $location = $base_uri . 'src/right_main.php?mailbox=' . urlencode($mailbox)
             . '&startMessage=' . (int)$startMessage;
   if (isset($srt)) {
       $location .= '&srt=' . (int)$srt;
   }
